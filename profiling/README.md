...
|---|---|
| `common.py` | Shared config: the "converged, ~6 track radii" grid (`grid_size_um=5.0, sampled_radius_cm=0.012`) and `run_once(num_threads)`, which returns timing + OS resource-usage (context switches, user/sys time) + scientific (`k_s`, `f_t`) metrics for one run. |
