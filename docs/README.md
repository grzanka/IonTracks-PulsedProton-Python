...
| [`RDD-CHARGE-VS-DOSE.md`](RDD-CHARGE-VS-DOSE.md) | **Does a radial *dose* distribution give the right radial *charge* distribution?** | You are using, or thinking about using, a tabulated RDD instead of the Gaussian. Reads Cucinotta (1996) against the libamtrack tabulation: the penumbra is an exact `1/r²` carrying 10.31 % of the LET per decade, but a third of the table is an excitation core that creates no ion pair at all — and dividing it by `W` puts phantom charge at the density peak. Ends with which RDD model to use, and why the multi-track case is the easy one. |
